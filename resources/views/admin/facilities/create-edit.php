    ...
    <div class="main-content-wrap">
        <div class="flex items-center flex-wrap justify-between gap20 mb-27">
            ...
            <ul class="breadcrumbs flex items-center flex-wrap justify-start gap10">
                ...
                <li>
                    <a href="{{ route('admin.facilitiess') }}">
                        <div class="text-tiny">facilitiess</div>
                    </a>
                </li>
                <li>
                    <i class="icon-chevron-right"></i>
                ...
                <li>
                    <div class="text-tiny">Edit facilities</div>
                </li>
            </ul>
        </div>

        <!-- form-edit-facilities -->
        <form action="{{ route('admin.facilities.update', $facilities->id) }}" class="tf-section-2 form-edit-facilities" method="POST" enctype="multipart/form-data">
            ...
            <div class="wg-box">
                ...
                {{-- Name --}}
                <fieldset class="name">
                    <div class="body-title mb-10">facilities name <span class="tf-color-1">*</span></div>
                    <div class="select mb-10">
                        <select name="name" id="facilitiesNameSelect" required>
                            <option value="">Select a facilities name</option> 
                            <option value="Male Dormitory" {{ $facilities->name == 'Male Dormitory' ? 'selected' : '' }}>Male Dormitory</option>
                            <option value="Female Dormitory" {{ $facilities->name == 'Female Dormitory' ? 'selected' : '' }}>Female Dormitory</option>
                            <option value="International House II" {{ $facilities->name == 'International House II' ? 'selected' : '' }}>International House II</option>
                            <option value="International Convention Center" {{ $facilities->name == 'International Convention Center' ? 'selected' : '' }}>International Convention Center</option>
                            <option value="Rolle Hall" {{ $facilities->name == 'Rolle Hall' ? 'selected' : '' }}>Rolle Hall</option>
                            <option value="Swimming Pool" {{ $facilities->name == 'Swimming Pool' ? 'selected' : '' }}>Swimming Pool</option>
                        </select>
                    </div>
                </fieldset>
                @error('name') 
                ...
                <fieldset class="description">
                    ...
                    <textarea class="mb-10" name="description" required>{{ $facilities->description }}</textarea>
                </fieldset>
                ...
                <fieldset class="rules_and_regulations">
                    ...
                    <textarea class="mb-10" id="rules" name="rules_and_regulations" placeholder="rules_and_regulations" required>{{ old('rules_and_regulations', $facilities->rules_and_regulations) }}</textarea>
                </fieldset>
                ...
                <fieldset>
                    ...
                    <div class="upload-image flex-grow">
                        <div class="item" id="requirementsPreview" style="display: {{ $facilities->requirements ? 'block' : 'none' }};">
                            @if($facilities->requirements)
                                <a href="{{ asset('uploads/facilitiess/'.$facilities->requirements) }}" target="_blank">{{ $facilities->requirements }}</a>
                            @endif
                            <button type="button" class="remove-upload {{ $facilities->requirements ? 'show' : '' }}" onclick="removeUpload('requirementsPreview', 'requirementsFile')">Remove</button>
                        </div>
                        <div id="upload-requirements" class="item up-load">
                            <label class="uploadfile" for="requirementsFile">
                                ...
                                <input type="file" id="requirementsFile" name="requirements" accept=".pdf,.doc,.docx,.jpg,.png" {{ $facilities->requirements ? '' : 'required' }}>
                            </label>
                        </div>
                    </div>
                </fieldset>
            ...
            <div class="wg-box">
                ...
                <fieldset>
                    ...
                    <div class="upload-image flex-grow">
                        <div class="item" id="imgpreview" style="position: relative; display: {{ $facilities->image ? 'block' : 'none' }};">
                            <img src="{{ asset('uploads/facilitiess/'.$facilities->image) }}" id="preview-img" class="effect8" alt="{{ $facilities->name }}">
                            <button type="button" class="remove-upload {{ $facilities->image ? 'show' : '' }}" onclick="removeUpload('imgpreview', 'myFile')">Remove</button>
                        </div>
                        <div id="upload-file" class="item up-load">
                            <label class="uploadfile" for="myFile">
                                <span class="icon"><i class="icon-upload-cloud"></i></span>
                                <span class="body-text">Select your main image here or click to browse</span>
                                <input type="file" id="myFile" name="image" accept="image/*" {{ $facilities->image ? '' : 'required' }}>
                            </label>
                        </div>
                    </div>
                </fieldset>
                ...
                <fieldset>
                    ...
                    <div class="upload-image mb-16 flex-grow" id="gallery-container">
                        @if($facilities->images)
                            @foreach(explode(",", $facilities->images) as $img)
                                <div class="item gitems">
                                    <img src="{{ asset('uploads/facilitiess/'.trim($img)) }}" style="width: 100px; height: 100px; object-fit: cover;" />
                ...
                <div id="priceFields" style="display: block;">
                    <fieldset class="name">
                        ...
                        <input type="number" name="price" value="{{ $facilities->price }}" required>
                    </fieldset>
                </div>
                ...
                <div id="internalExternalPriceFields" style="display: none;">
                    <fieldset class="name">
                        ...
                        <input class="mb-10" type="number" placeholder="Enter capacity" name="capacity" value="{{ $facilities->capacity }}" required>
                    </fieldset>
                    ...
                    <div class="cols gap22">
                        <fieldset class="name">
                            ...
                            <input class="mb-10" type="text" placeholder="Enter internal price" name="internal_price" value="{{ $facilities->internal_price }}" required>
                        </fieldset>
                        ...
                        <fieldset class="name">
                            ...
                            <input class="mb-10" type="text" placeholder="Enter external price" name="external_price" value="{{ $facilities->external_price }}" required>
                        </fieldset>
                        ...
                        <fieldset class="name">
                            ...
                            <input class="mb-10" type="text" placeholder="Enter exclusive price" name="exclusive_price" value="{{ $facilities->exclusive_price }}">
                        </fieldset>
                ...
                <div class="cols gap22">
                    <fieldset class="name">
                        ...
                        <div class="select mb-10">
                            <select name="status">
                                <option value="available" {{ $facilities->status == 'available' ? 'selected' : '' }}>Available</option>
                                <option value="not available" {{ $facilities->status == 'not available' ? 'selected' : '' }}>Not Available</option>
                            </select>
                        </div>
                    </fieldset>
                    ...
                    <fieldset class="name">
                        ...
                        <div class="select mb-10">
                            <select name="featured">
                                <option value="0" {{ $facilities->featured == 0 ? 'selected' : '' }}>No</option>
                                <option value="1" {{ $facilities->featured == 1 ? 'selected' : '' }}>Yes</option>
                            </select>
                        </div>
                    </fieldset>
                ...
                <div class="cols gap10">
                    <button class="tf-button w-full" type="submit">Update facilities</button>
                </div>
            </div>

    <!-- Conditional Fields for Dormitory facilitiess (Male Dormitory, Female Dormitory, International House II) -->
    <div class="wg-box" style="display: {{ in_array($facilities->name, ['Male Dormitory', 'Female Dormitory', 'International House II']) ? 'block' : 'none' }}">
        <div id="dormitoryFields">
            <h4>Room Details</h4>
            <div id="roomContainer">
                @if($facilities->dormitoryRooms)
                    @foreach($facilities->dormitoryRooms as $room)
        ...
        <div class="d-grid gap10 mt-5">
            <button class="dorm-btn btn-primary w-full" type="submit">Update facilities</button>
        </div>
    </div>
...
<script>
    ...
    // Function to toggle required fields based on facilities type
    function togglePriceFields() {
        const selectedName = $('#facilitiesNameSelect').val();
        ...
        // facilities types that require only the price field
        const priceRequiredfacilitiess = ['Male Dormitory', 'Female Dormitory', 'International House II'];

        // facilities types that require capacity, internal price, and external price fields
        const internalExternalRequiredfacilitiess = ['International Convention Center', 'Rolle Hall', 'Swimming Pool'];

        // Toggle fields based on facilities type
        if (priceRequiredfacilitiess.includes(selectedName)) {
            priceField.show();
        ...
        } else if (internalExternalRequiredfacilitiess.includes(selectedName)) {
            priceField.hide();
            ...
            // Hide all fields if the facilities type doesn't match any condition
            priceField.hide();
            internalExternalPriceFields.hide();
    ...
    $(document).ready(function() {
        ...
        // Toggle fields when the facilities type selection changes
        $('#facilitiesNameSelect').on('change', togglePriceFields);
    ...
    function toggleUpdateButton() {
        ...
        // List of facilities names to hide the button
        const restrictedfacilitiess = ['Male Dormitory', 'Female Dormitory', 'International House II'];

        if (restrictedfacilitiess.includes(selectedName)) {
    ...
    $(document).ready(function() {
        ...
        // Attach the toggle function to the facilities name dropdown change event
        $('#facilitiesNameSelect').on('change', toggleUpdateButton);
    });
    ...
    $(document).ready(function() {
        ...
        $('.form-edit-facilities').on('submit', function(e) {
